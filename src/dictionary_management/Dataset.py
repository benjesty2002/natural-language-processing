...
class Dataset():
    def __init__(
        ...
        self.file_path, self.directory, self.file_name, self.file_type = self.__expand_file_path(file_path)
        self.source = source
        self.url = url
        self.update = update_method if update_method else self.__generate_update_method()
    ...
    def __generate_update_method(self) -> Callable:
        if not self.url or not self.file_name:
            raise RuntimeError("url and filename must be specified if no update method is provided")
        match self.source:
